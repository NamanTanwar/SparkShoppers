...
import { sendOtp, resendOtpToUser } from "../../../services/operations/authAPI";
...
const OTPInput = ({ length = 6 }) => {
  ...
  const { tempOtpId } = useSelector((state) => state.auth);
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(sendOtp(otp, tempOtpId, navigate));
  };

  const resendOtp = () => {
    dispatch(resendOtpToUser(tempOtpId));
  };
