...
import { FaShoppingBag, FaArrowUp, FaArrowDown } from "react-icons/fa";
import { Line } from "react-chartjs-2";

const SalesTab = () => {
  const { salesComparisonData } = useSelector((state) => state.admin);

  const { currentMonthSales, percentageChange } = salesComparisonData || {
    currentMonthSales: 0,
    percentageChange: 0,
  };
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center space-x-4">
          <div className="bg-blue-100 dark:bg-blue-900 p-3 rounded-full transition-colors duration-300">
            <FaShoppingBag className="text-blue-500 dark:text-blue-400 text-2xl" />
          </div>
          <div>
            ...
            <p className="text-3xl font-semibold text-gray-800 dark:text-white mt-1 transition-colors duration-300">
              Rs. {currentMonthSales?.toLocaleString() || "0"}
            </p>
          </div>
        </div>
