export const calculateAverageRating = (ratingAndReviews) => {
    ...
    const averageRating = (ratingAndReviews.reduce((sum, ratingAndReview) => sum + ratingAndReview.rating, 0) / ratingAndReviews.length).toFixed(1);
    ...
    for (const [key, value] of averageRatings) {
        averageRatings.set(key, ((value / ratingAndReviews.length) * 100).toFixed(1));
    }
