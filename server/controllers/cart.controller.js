...
const addToCart = async (req, res) => {
  try {
    ...
    const { productId, productOptions, quantity } = req.body;

    const userToken = req.headers.authorization.split(" ")[1];

    const userId = await tokenService.extractUserIdFromToken(userToken);

    const userCart = await cartService.addToCart(
      userId,
      productId,
      productOptions,
      quantity
    ...
    if (
      err.message === "Item quantity limit reached" ||
      err.message === "Item out of stock"
    ) {
...
const removeFromCart = async (req, res) => {
  try {
    ...
    const { productId, removeItem } = req.body;
    //extracting userId from userToken
    const userToken = req.headers.authorization.split(" ")[1];
    const userId = await tokenService.extractUserIdFromToken(userToken);
    //Removing item from cart
    const userCart = await cartService.removeFromCart(
      userId,
      productId,
      removeItem
    );
